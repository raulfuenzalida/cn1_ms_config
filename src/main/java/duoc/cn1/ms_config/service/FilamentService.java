...
public class FilamentService {
	...
	public FilamentResponse createFilament(FilamentCreateRequest request) {
		if (filamentRepository.existsByNameAndColor(request.getName(), request.getColor())) {
			throw new FilamentDuplicateException(request.getName(), request.getColor());
		}
		...
		Filament savedFilament = filamentRepository.save(filament);
		return mapToResponse(savedFilament);
	}

	public FilamentResponse updateFilament(Long id, FilamentUpdateRequest request) {
		Filament filament = filamentRepository.findById(id)
			.orElseThrow(() -> new FilamentNotFoundException(id));

		if (!filament.getName().equals(request.getName()) || 
		...
		if (request.getPricePerKg() != null) {
			filament.setPricePerKg(request.getPricePerKg());
		}

		Filament updatedFilament = filamentRepository.save(filament);
		return mapToResponse(updatedFilament);
	}

	public FilamentResponse updateFilamentStatus(Long id, FilamentStatusUpdateRequest request) {
		Filament filament = filamentRepository.findById(id)
			.orElseThrow(() -> new FilamentNotFoundException(id));
